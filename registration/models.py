...
class TechTestManager(models.Manager):
    def create_test_entry(self, student, test, marks):
        return self.create(students=student, test=test, marks=marks)

class TechTest(models.Model):
    test = models.ForeignKey( Test, null=False, blank=False)
    students = models.ForeignKey(Student, null=False, blank=False)
    marks = models.FloatField(_('Mark'), null=False, blank=False)
